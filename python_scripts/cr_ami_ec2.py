...
# This Python Script is designed to create, copy and delete
# AMIs/associated snapshots for the ODS Infrastructure
...
image_east = ec2_source.describe_images(
    Filters=[{'Name': 'tag:Stack', 'Values': ['Development']}])['Images']
image_west = ec2_destination.describe_images(
    Filters=[{'Name': 'tag:Stack', 'Values': ['Development']}])['Images']
snaps_east = ec2_source.describe_snapshots(
    Filters=[{'Name': 'tag:Delete', 'Values': ['30days']}])['Snapshots']
snaps_west = ec2_destination.describe_snapshots(
    Filters=[{'Name': 'tag:Delete', 'Values': ['30days']}])['Snapshots']
...
for image in image_west:
    ...
    if day_old > age:
        ...
        print("There are no AMIs available with the Development tag in Oregon")
...
def create_ami(image_east, image_west):
    ...
    reservations = ec2_source.describe_instances(
        Filters=[{'Name': 'tag:Stack',
                  'Values': ['Development']}])['Reservations']
    instances = []
